...
export function useAssets(initalLimit =10) {
  ...
  const [limit] = useState(initalLimit)
  ...
  const fetchVehicles = useCallback(async () => {
    ...
    try {
      ...
      // Assuming res = { vehicles: [], total: 100 }
  ...
  return { vehicles, loading, error, page, limit, total, setPage, fetchVehicles };
  ...
  const addVehicle = async (payload: any) => {
    ...
    try {
      ...
      setVehicles((prev) => [...prev, data]); // Append newly added vehicle
      return data;
    } catch (err: any) {
